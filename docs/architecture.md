...
## 1. 시스템 목적
이 프로젝트는 영어 단어 목록을 서버에서 배치 처리하여 Google Ads historical metrics를 수집하고, 결과를 날짜별 CSV/XLSX로 export한 뒤 Google Drive와 Google Sheets에 동시 배포하는 구조를 만든다.
...
## 4. 시스템 구성요소
- Cloudflare Workers Free: 배치 실행 엔진
- Cloudflare Cron Triggers: 주기 실행
- Cloudflare D1 Free: shard/batch/export/publish 상태 저장
...
### 6.4 Drive 업로드 및 상태 등록
규칙:
- 모든 shard와 manifest를 Google Drive 원본 폴더에 업로드
- D1에 초기 상태 `pending` 등록
- 미등록 shard가 없어야 한다

### 6.5 Cron 기반 batch 실행
배치 절차:
...
### 6.7 결과 저장
원칙:
- 결과 원본 파일은 Google Drive에 저장
- 상태/인덱스/재시도 메타는 D1에 저장
